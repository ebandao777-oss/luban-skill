...
L3 价值对齐层在不同执行模式下表现为不同的约束形式，其具象化落在两点（Quick 软性 + Full 硬性）：

**Quick 模式 — 软性约束**：从 `luban-profile.json` 的 `style_tags` 中读取用户风格偏好（如"中文为主、简洁为上"），以 dim7（整体架构）的评分权重调整形式生效——若 SKILL.md 偏离 style_tags 预设风格，dim7 权重从 12 调整为 14（即偏离加剧扣分更严厉），反之若高度对齐则 dim7 权重降为 10（即风格对齐换取更宽松的结构容忍度）。该约束为软性的——偏离仅在评分上体现，不阻断优化过程。

**Full 模式 — 硬性约束**：落地为 dim8 多维仪表盘的两个子维度：

| 子维度 | 权重(dim8内) | 检测范围 | 熔断 |
|:---|:---|:---|:---|
| Safety（安全性） | 5 | 删除操作 / 文件覆盖 / 凭据泄露检测 / 系统调用审计 / 路径注入 | 任一命中 → 冻结优化 |
| Compliance（合规率） | 5 | 格式规范 / 无幻觉路径 / 输出中立性 / 术语一致性 | < 80% → 否决本轮 |

Safety 命中具有一票否决权——无论其他维度得分再高，只要 Safety 任一子项命中红线，立即冻结该 skill 的 L2 优化权限并触发 `l4.freeze`。Compliance 未达 80% 则本轮优化成果无效，恢复旧版本。
...
for each skill in 优化范围:
  ...
  **重测回路运维参数**：
  ...
  | 参数 | 值 | 说明 |
  |:---|:---|:---|
  | ping 间隔 | 30s | 消费者基线健康度检查的心跳间隔——每 30 秒对目标模型发送一次裸能力探测（不含 skill） |
  | pong 超时 | 180s | 消费者响应时间上限——超过 180 秒未收到有效响应的探测视为失败，计入性能波动计数 |
  | ping 超时 | 120s | 心跳总超时——单次 ping 连接建立 + 首 token 响应的最大等待时间 |
  | 重试上限 | 3 次 | pong 超时 / ping 超时的最大重试次数——连续 3 次失败后触发「消费者不可达」告警，暂停优化
...
for each skill:
  ...
  while round < MAX_ROUNDS (默认3):
    ...
    # === 备份容量管理与版本保护 ===
    **备份容量管理**：
    - **版本上限**：每个 skill 的 luban-backups/ 下最多保留 50 个版本文件（mtime 基准排序）。超出上限时，按 LRU 逐出最旧的非 keep 版本
    - **LRU 淘汰基准**：以文件的最后修改时间（mtime，通过 `os.path.getmtime()` 获取）作为"最近使用"的时间基准——最近未修改即最近未"使用"
    - **淘汰记录**：每次淘汰操作写入 `version_manifest.json` 的 `pruned` 数组，记录 `{file, mtime, reason: "LRU_cap_50", timestamp}`

    **keep 版本保护规则**：
    - 所有在 Phase 2 决策中标记为 `keep` 的版本（results.tsv status=keep 对应的备份文件）受**直到非 keep 版本不足 10 个才可淘汰**的保护
    - 淘汰优先级：非 keep 旧版本 → 非 keep 新版本 → keep 旧版本（仅在非 keep < 10 时允许淘汰 keep，且淘汰 mtime 最早的 keep 版本）

    **极端饱和逐出策略**（磁盘空间低于最小阈值时触发，3 级降级）：
    1. **时间衰减**（优先）：对非 keep 版本计算衰减权重 `w = 1 / (1 + days_since_mtime/30)`，逐出最旧的非 keep 版本，直至磁盘空间恢复
    2. **k-means 代表性快照**（时间衰减后仍不足）：对剩余版本按 mtime 聚为 k=5 簇，每簇保留离质心最近的 1 个版本来压缩历史
    3. **最终兜底**（仍不足）：超出容量上限的部分标记 `force_evicted` 并写入 `version_manifest.json` pruned 数组，发出 `storage_critical` 告警

    ## V2 哈希链断裂恢复协议

    `version_manifest.json` 的 V2 审计链为链式 SHA-256（每条记录含 `prev_hash`），校验时从创世记录向后逐条验证。若位置 k 的哈希校验失败，按以下协议恢复：

    1. `record_k` 及之后所有记录 > 标记 `integrity=invalid`，记录位置 k 的预期哈希与实际哈希
    2. `record_{k-1}` 及之前的所有记录**维持 `integrity=valid`**——哈希链断裂前段仍可信（这是链式哈希的核心优势：前缀不可篡改）
    3. **逐记录重算修复**：从 `record_{k-1}` 的哈希出发，依次重新计算 `record_k, record_{k+1}, ...` 的哈希值并更新 `prev_hash`，同时标注 `integrity=repaired` + `repaired_at=ISO8601`
    4. **创世记录失败**（k=0）：若第一条记录的哈希即校验失败 → 整个审计链不可信 → 标记 `audit_chain_root_corrupt`，触发 `l4.alert`，停止当前优化
    5. 修复完成后写入 `integrity_repair_report` 到 `meta_learnings.md`「异常信号」章节，包含：修复时间、位置 k、断裂数量（N-k 条）、修复后校验结果
...
SA-DM §四 定义 8 条架构红线，使优化有底线、不可绕过：

| # | 红线 | 说明 |
|:---|:---|:---|
| #1 | **禁止改评同源** | 同一 agent 不能既做优化改文件，又给自己的改动打分——dim8 必须由独立子 agent 或干跑验证执行 |
| #2 | **禁止优化>编辑** | 单轮 token 变化量不得超过原文件的 10%（编辑预算），超出必须拆分多轮 |
| #3 | **禁止绕过回滚** | 任何改动必须有回滚路径——备份在改动前完成，回滚必须用文件覆盖而非直接删除 |
| #4 | **禁止跨模式污染** | quick 模式的训练数据不得用于 full 模式评估（反之亦然），results.tsv 和备份分存储桶物理隔离 |
| #5 | **禁止丢弃负反馈** | 被 revert 的编辑必须写入 rejected_edits.md 永久保留，后续轮次审阅后必须检查并绕行 |
| #6 | **禁止过拟合生产** | held-out 比 main 更关键——held-out 退化的“+10 分版本”不如 held-out 不变的“+0 分版本” |
| #7 | **禁止幻觉评分** | 每条评分的理由必须有原文锚定——无锚定评分视为疑似幻觉，dim8 直接标 dry_run 不可信 |
| #8 | **禁止跳过复盘** | Full 模式下完成 ≥2 个 skill 优化后必须执行 Epoch Meta-Review——不复盘就不算闭环 |

#### 红线运行时检测

8 条红线中，仅 #4 和 #8 具有运行时自动检测能力，其余 6 条为声明性约束（依赖 Phase 2 执行纪律 + 反模式表人工对照）：

| 红线 | 检测方式 | 违规动作 |
|:---|:---|:---|
| #4 跨模式污染 | Phase 0 检查 results.tsv 和备份目录的模式隔离：若发现 quick 模式 results 与 full 模式备份目录存在交叉引用 | 阻断本轮优化并通知 |
