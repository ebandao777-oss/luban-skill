...
| dim5 可执行具体性 | 17 | LLM | 有具体参数/格式/示例；前置扫描软化词（建议/可考虑/根据情况/灵活把握/视情况而定/可能/大概）注入评分上下文。≥5 处软化词扣 ≥3 分 |
...
```
    ...
    │ EvoSkill 失败驱动   │ 扣分 │  Phase 0   初始化   │      │  错误反馈 → EvoSkill  │
...
### 模块扣分规则（Phase 0.3 产出 → Phase 1 扣分）

模块不产生评分，仅产出缺陷检测结果。每笔扣分在 `diagnostics.tsv` 中记录（模块/维度/扣分/文件/行号/详情），Phase 1 从 Rubric 原始分扣除。

| 模块 | 检测项 | 扣分规则 | 关联维度 |
|:---|:---|:---|:---|
| SkillOps | 断裂引用（路径不存在） | −2/处，上限 −6 | dim6 |
| SkillOps | YAML/Frontmatter 非法 | −3/处 | dim1 |
| SkillOps | 引用文件列表中存在但未被 SKILL.md 使用 | −1/处 | dim6 |
| HASP | 软化词 ≥5 处 | −2 | dim5 |
| HASP | 违规计数 ≥2 次（同一规则连续被忽略） | −1/次，上限 −3 | dim10 |
| CASCADE | 外部引用过期 >180 天 | −1/处 | dim6 |
| Distill | F_approx ≥0.7 文件 ≥2 个 | −2 | dim7b |
| Distill | 完全未被引用的 references | −1/文件，上限 −4 | dim7b |
| EvoSkill | 历史振荡（同维度 2+ 轮反复涨跌） | 仅标注 `[oscillation]`，**不自动扣分** | dim3 |
| Sentinel | 恶意指令（exec/system/subprocess/rm−rf/format 等） | −2/类，上限 −4 | dim10 |
| Sentinel | 硬编码凭据（API Key/密码/token 明文） | −2/处，上限 −4 | dim10 |
| Sentinel | Prompt 注入模式（DAN/越狱/simulate/system override 等） | −2/处 | dim10 |
| Sentinel | 数据外泄路径（SMTP/上传外网/SCP 等） | −2/处 | dim10 |
| Sentinel | 权限越权（chmod/chown/sudo/icacls 等） | −2/处 | dim10 |

扣分上限：dim6 −6、dim5 −4、dim7 −4、dim10 −8、dim1 −3。扣后维度分下限 0。
...
命中任一项则打断流程，报告具体违规位置（行号+原文），要求修复后重新进入 Phase 1。免检白名单：skill name 明确绑定单一 runtime。
...
7. 检查 diagnostics.tsv 是否存在，不存在则创建并写入表头（模块/维度/扣分/文件/行号/详情）
```

### Phase 0.3: 模块缺陷检测（扣分清单）

六模块对目标 skill 做静态检测，输出缺陷扣分到 diagnostics.tsv。**模块不产生评分，只产扣分**。

```
for each skill:
  1. 按优先级顺序执行：SkillOps → EvoSkill → HASP → CASCADE → Distill → Sentinel
  2. 每模块按「模块扣分规则」逐项检测
  3. 命中缺陷 → 追加 diagnostics.tsv 一行（模块/维度/扣分/文件/行号/详情）
  4. 模块完成后不回写 SKILL.md，仅写入扣分记录
```

EvoSkill 例外：振荡检测仅标注 `[oscillation]`，不自动扣分，供 Phase 2 Step 1 人工决策。
...
🔴 CHECKPOINT：展示扣分摘要（各维度扣了多少、哪些模块有输出、EvoSkill 振荡告警、Sentinel 安全告警），确认后进入 Runtime 中立性 Gate → Phase 0.5。
...
Rubric 是**唯一总分来源**。流程：先按 Rubric 逐维评分得原始分 → 再从原始分扣除 diagnostics.tsv 中模块检测的缺陷扣分 → 得最终分。

```
for each skill:
  1. 按 Rubric 表逐维评分，得各维度原始分（模块不参与评分）
  2. 读取 diagnostics.tsv，按维度汇总扣分
  3. 各维度最终分 = max(0, 原始分 − 该维累计扣分)
...
模块扣分清单（Phase 0.3 产出），每模块对目标 skill 的静态缺陷检测结果：

```
模块	维度	扣分	文件	行号	详情
SkillOps	dim6	-2	SKILL.md	120	引用 references/xxx.md 不存在
HASP	dim5	-2	SKILL.md	-	软化词≥5 处（建议×3/可考虑×1/根据情况×1）
EvoSkill	dim3	0	-	-	[oscillation] dim5 历史 3 轮回弹
```

- 文件位置：被优化 skill 目录下的 `diagnostics.tsv`
- Phase 0.3 每次运行前**清空重建**（保持最新一轮扣分）
- Phase 1 读扣分按维度汇总，从 Rubric 原始分扣除
- EvoSkill 不扣分，`扣分=0`，详情标注 `[oscillation]`
