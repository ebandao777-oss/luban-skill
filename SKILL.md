...
L2→L4 跨层接口：`l4.rollback`（原子 rename 回滚）、`l4.freeze`（冻结 L2 优化权限）、`l4.budget_tighten`（全局收紧编辑预算）、`l4.alert`（异常事件通知）。回滚采用 rename 原子操作（<50ms），失败时降级为 copy+unlink 兜底。

完整分层定义、L0 接口规范、L1 MVS 接口、回滚失败回退策略、只读保护模式退出条件见 `references/SA-DM.md` §五.2。
...
| 3 | **失败模式编码** | 11 | 可靠性 | **必须显式编码失败模式**（写出"如果 X 失败 → Y"的明确分支）；有fallback路径、错误恢复；**只写正向流程而不写失败分支扣 ≥3 分**（SkillLens meta-skill 维度） |
| 4 | **检查点设计** | 6 | 可信任度 | 关键决策前有用户确认、防止自主失控；**检查点必须显性标记（🔴/STOP/CHECKPOINT），仅靠"如果...建议..."措辞不算** |
| 5 | **可执行具体性** | 18 | 有效性 | 不模糊、有具体参数/格式/示例、可直接执行；**禁止"建议/可以考虑/根据情况/灵活把握/视情况而定"等软化措辞**——出现 ≥3 处扣 ≥3 分（SkillLens actionable specificity 维度） |
...
| # | 维度 | 权重 | 架构五维映射 | 评分标准 |
|---|------|------|:---:|---------|
| 8 | **实测表现** | 22 | 有效性+可信任度+规范性 | 跑2-3个 test prompt（含主流/边界/异常三类场景），从 Accuracy/Safety/Compliance/Latency/Token Efficiency 五个子维度综合打分 |
...
| 10 | **审查门控度** | 5 | 可信任度+可靠性+规范性 | 多角色审查中 P0/P1/P2 严重度门控通过情况：基础分 100，P0 每项未闭合 −30 且上限锁死 40（任一 P0 未闭合即触发，先扣 P0/P1 分后再应用锁死上限），P1 每项未闭合 −5，P2 不扣分，下限 0。dim10 得分（归一化）= dim10 原始分(0~100) / 10 → 映射到 1~10 区间后参与加权计算。Quick 模式不执行 dim10（默认满分 100，归一化后 10 分） |
...
若子 agent 不可用（超时/资源限制），退化为「干跑验证」：读完 skill 后模拟一个典型 prompt 的执行思路，判断流程是否合理；必须在 results.tsv 标注 `dry_run`。**dry_run 比例 > 30% → 评估失效警告**（来自本机 controlled study：dim8 实测维度权重 22%，无 full_test 验证时分数不可信）。
...
同一 10 维量规在 Quick/Full 模式下有不同的执行深度：

| 维度 | Quick 模式 | Full 模式 |
|:---|:---|:---|
| dim8 执行方式 | dry_run 推演（占比 ≤30%） | full_test 子 agent 实测（占比 ≥50%） |
| dim10 审查门控度 | 不执行（默认满分 100，归一化后 10 分） | P0/P1/P2 全量门控，任一 P0 未闭合→dim10 锁死 ≤40 |
...
基于 SA-DM §五.1 定义的 5 参数加权自动路由，在用户手动选择模式前先给出智能推荐。冷启动期（record_count<5）不生效，直接走手动选择面板。

| 参数 | 归一化函数 N(x)→[0,1] | 权重 |
|:---|:---|:---:|
| ① 任务领域 | 金融/医疗/法律→1.0；生产部署→0.8；技术方案→0.4；个人创作→0.0；未命中→0.2 | 0.30 |
| ② 数据敏感度 | 身份证/银行卡/密钥→1.0；内网路径→0.7；邮箱/手机→0.3；无→0.0 | 0.25 |
| ③ 文件规模 | SKILL.md >800行或≥5引用文件→1.0；(200,800]→线性映射；≤200→0.0 | 0.20 |
| ④ 历史 revert 率 | revert≥3→1.0；=2→0.7；=1→0.3；=0→0.0 | 0.15 |
| ⑤ 用户显式覆盖 | 检测到 quick/full → 短路（M=0.0/1.0），不计算其他参数 | 短路段 |

**路由决策表**（M = Σ(wᵢ × N(xᵢ))）：

| M 区间 | 动作 | 用户感知 |
|:---|:---|:---|
| ≥0.90（硬路由） | 自动执行 Full 十维量规，不暂停 | 下次会话 Phase 0 告知 |
| [0.70, 0.90)（软路由） | 建议 Full，暂停确认 | 展示选择理由 |
| (0.30, 0.70)（待决策） | 展示理由，用户手动选 | Quick/Full 面板 |
| (0.10, 0.30]（软默认） | 默认 Quick，可一键切换 | 轻提示 |
| <0.10（硬默认） | 自动 Quick，无感知 | 无 |

边界 rounding：M 保留 4 位，路由判定四舍五入到 2 位。0.90 归硬路由，0.30 归软默认，0.10 归软默认（仅 ≤0.09 才硬默认）。误判纠偏：连续 2 次手动覆盖同一方向 → 领域倾向权重衰减 50%，记录豁免。

完整定义见 `references/SA-DM.md` §五.1。
...
**干跑模式控制**：dry_run 比例 >30% → dim8 标注可信度不足；连续 2 次 >50% → 判别力退化告警，全量 full_test 重建基线。滞回控制：重建后锁定 full_test ≥30% 持续 5 会话。dim8 子维度 Safety 豁免 dry_run/full_test 偏差对比，独立走规则扫描+退化检测。

完整定义见 `references/SA-DM.md` §五.4。
...
告警升级路径：同一类型连续 3 次 → 暂停优化生成问题报告 → 第 5 次冻结 L2 权限。自动恢复：冷却期（5/10 轮）无重复触发 → 降级。完整退化检测协议+自动恢复闭环见 `references/SA-DM.md` §十。
...
- **train.py** → 每个SKILL.md
- **val_bpb** → 10维加权总分（含多维仪表盘 + meta-skill 反例黑名单 + 审查门控度）
- **ratchet** → 只保留有改进的版本
...
| 10. 审查门控度 | {dim10_old} | {dim10_new} | {Δ10} |
