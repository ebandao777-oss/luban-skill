...
| EvoSkill | 失败捕获记录扫描：从 results.tsv 提取历史失败维度 | dim3 |
...
for each skill:
  ...
  2. 模块未覆盖的维度（dim1/dim2/dim4/dim9）由主 agent 按 Rubric 补充评分；dim8 唯一来自效果评分
...
for each skill（按基线分数升序，先优化最弱的）:
  ...
  while round < MAX_ROUNDS:
    ...
    # Step 0: 模块诊断（每轮重新执行）
    重新运行 Phase 0.3 五模块检测 → 更新 diagnostics.tsv
...
模块诊断记录（Phase 0.3 产出），每模块对目标 skill 的静态检测结果：

```
模块	维度	分数	文件	行号	详情
SkillOps	dim7	5	SKILL.md	120	缺少触发词章节
EvoSkill	dim3	6	-	-	历史 3 次 dim3 波动
```

- 文件位置：被优化 skill 目录下的 `diagnostics.tsv`
- Phase 0.3 每次运行前**清空重建**（保持最新一轮诊断）
- Phase 1 读数后按维度聚合取中位数
